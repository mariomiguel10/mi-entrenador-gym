...
const App: React.FC = () => {
  ...
  const handleStartGeneration = async () => {
    ...
    try {
      ...
      console.error(err);
      alert("No se pudo generar la rutina. Verifica tu conexión.");
      setView('config');
    }
  };
    ...
    <div className="max-w-xl mx-auto px-4 py-10 animate-in fade-in slide-in-from-bottom-6 duration-700">
      <div className="glass-card p-8 sm:p-12 rounded-[3rem] shadow-2xl">
        <header className="text-center mb-10">
          <div className="inline-block px-4 py-1.5 bg-indigo-50 text-indigo-700 rounded-full font-black text-[10px] uppercase tracking-widest mb-4">SmartFit AI Engine</div>
          <h1 className="text-4xl font-black text-slate-900 leading-tight">Tu Coach Personal</h1>
          <p className="text-slate-400 font-medium mt-2">Configura tu sesión perfecta</p>
        </header>

        <div className="space-y-10">
          <section>
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 block">¿Cuál es tu objetivo?</label>
          ...
          <div className="grid grid-cols-2 gap-6">
            <section>
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 block">Nivel</label>
              <select 
            ...
            <section>
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 block">Tiempo</label>
              <select 
                value={config.duration}
                onChange={(e) => setConfig({...config, duration: parseInt(e.target.value) as Duration})}
                className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-700 appearance-none"
              >
          ...
          <section className="bg-slate-50 p-6 rounded-[2.5rem] border border-slate-100">
             <div className="flex justify-between items-center mb-4">
                <span className="text-[10px] font-black text-slate-400 uppercase">Descanso series</span>
  ...
  const renderLoading = () => (
    <div className="flex flex-col items-center justify-center h-screen p-10 text-center animate-in fade-in duration-1000">
      ...
      <h2 className="text-4xl font-black text-slate-900 mb-4 tracking-tight">Diseñando tu sesión...</h2>
      <p className="text-slate-400 font-medium max-w-xs">Estamos seleccionando los mejores ejercicios de {config.objective.toLowerCase()} para ti.</p>
    </div>
  );
    ...
    <div className="max-w-2xl mx-auto px-4 py-10 animate-in zoom-in-95 duration-500">
      <div className="bg-white rounded-[3.5rem] shadow-2xl overflow-hidden border border-slate-100">
        <div className="bg-slate-900 p-10 text-white text-center">
          <span className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.4em] mb-2 block">Lista de Desafíos</span>
        ...
        <div className="p-8 sm:p-12 space-y-4">
          ...
          <button 
            ...
            className="w-full mt-8 py-6 bg-indigo-600 text-white rounded-[2rem] font-black text-xl shadow-xl shadow-indigo-100 transition-all transform active:scale-95"
          >
      ...
      {view === 'finished' && (
        <div className="max-w-lg mx-auto px-4 py-20 text-center animate-in zoom-in-90 duration-500">
          <div className="bg-white p-12 rounded-[4rem] shadow-2xl border border-emerald-50">
            ...
            <h2 className="text-5xl font-black text-slate-900 mb-4 tracking-tighter">¡Lo lograste!</h2>
            <p className="text-slate-400 mb-12 font-semibold">Has superado tu entrenamiento. Tu disciplina es tu mayor fuerza.</p>
            <button 
              onClick={() => setView('config')} 
              className="w-full py-6 bg-slate-900 text-white rounded-[2rem] font-black text-lg shadow-xl hover:bg-slate-800 transition-all"
            >
              Nueva Sesión
            </button>
          </div>
